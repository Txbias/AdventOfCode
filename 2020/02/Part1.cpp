...
bool isValid(std::string line) {
    ...
    int min = std::stoi(line.substr(0, indexDivider));
    int max = std::stoi(line.substr(indexDivider+1, line.find(" ")));
    ...
    return min <= letterOccurences && letterOccurences <= max;
}
